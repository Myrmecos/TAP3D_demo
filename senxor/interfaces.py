...
def discover_mi_usb():
    ...
    if not dev_list:
        raise RuntimeError('No MI48 connected')
    return dev_list, vcom_list
...
class TCPIP_Interface:
    ...
    def close(self):
        self.remote_server.shutdown(socket.SHUT_RDWR)
    ...
    def regread(self, reg, regname=""):
        """Read a control/status register via TCPIP protocol"""
        result = None
        while result is None:
            cmd = 'RREG{:02X}XXXXXX'.format(reg)
            cmd = '   #{:04X}{}'.format(len(cmd), cmd)
            cmd_name = 'GET_{}'.format(regname)
            result = tcpip_command(self.remote_server, cmd, cmd_name)
            if result is None: return
            if not isinstance(result, int):
                # a non int would be a GFRA coming back before the RREG
                result = None
    ...
    def regwrite(self, reg, value, regname=""):
        """Write to a control register via TCPIP protocol"""
        cmd = 'WREG{:02X}{:02X}XXXX'.format(reg, value)
        cmd = '   #{:04X}{}'.format(len(cmd), cmd)
        cmd_name = 'SET_{}'.format(regname)
        tcpip_command(self.remote_server, cmd, cmd_name)
        return None
    ...
    def read(self, size_in_words):
        ...
        The returned data frame is a 1-D numpy array of unsigned int16.
        """
        cmd, data = tcpip_acknowledge(self.remote_server)
        if cmd == 'GFRA':
            # data is a sequence (1-d array) of 16-bit unsigned ints
            # here we drop the USB header
            return data[-size_in_words:]
...
def tcpip_command(remote_server, cmd: str, cmd_name='', verbose=True):
    """send command to MI48 via USB and return its acknowledge"""
    _cmd = ''
    while _cmd != cmd[8:12]:
        # host command
        remote_server.send(cmd.encode())
        # device ack
        _cmd, data = tcpip_acknowledge(remote_server)
        if _cmd != cmd[8:12]:
            if verbose:
                logger.debug('Expected ACK: {}, rcvd: {}'.
                             format(cmd[8:12], _cmd))
...
def tcpip_acknowledge(remote_server):
    """Receive the EVK acknowledge and parse it"""
    ack = None
    # this loop will make the program hang if ser.read()
    # has no timeout configured!
    while ack is None:
        # if *.decode() yields UnicodeDecodeError
        # drop the ACK and wait for the next one
        ack = tcpip_get_ack(remote_server)
    parsed = tcpip_parse_ack(*ack)
    return parsed

def tcpip_parse_ack(cmd:str, data:bytes):
    """
    Parse command and return the command string and a data item.
...
def tcpip_get_ack(remote_server):
    """
    Obtain an acknowledge to a command sent to a virtual serial port
    ...
    Return bytes.
    """
    databuffer = remote_server.recv(10256)
    res = databuffer[0:4]
    ...
    cmd = databuffer[8:12]
    cs = cksum(cmd, cs)
    data = databuffer[12:12+data_len]
    if data_len > 0:
        cs = cksum(data, cs)
    cs = cs & 0xFFFF
    # Read the check sum field
    cks = databuffer[12+data_len:12+data_len+4]
    try:
        cks.decode()
    except UnicodeDecodeError:
        print('TCPIP check sum decode error: {}'.format(cks))
        return None
    try:
        cks = int(cks, base=16)
    except ValueError:
        # if host too slow, we get invalid literals here
        logger.error('Bad TCPIP check sum literals for {}: {}'.format(cmd, cks))
        return None
    if cs != cks:
        logger.error('Check sum mismatch: calculated {}, received {}'.
                    format(hex(cs), hex(cks)))
