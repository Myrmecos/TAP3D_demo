...
DEFAULT_CTRL_STAT = {
    ...
    'POWER_DOWN_1': 0x00,
    'POWER_DOWN_2': 0x02,
    "SENS_FACTOR" : 0x64,  # read from module; ideally 0x64 = 1.00
    'EMISSIVITY': 0x5F,
    'OFFSET_CORR': 0x00,   # read from module, but we should show what's used always
    'OBJ_TEMP_FACTOR': 0x00,  # module dependent, but we should show what's used always
    'FILTER_1': 0x00,
    'FILTER_1_LSB': 0x0A,
    'FILTER_1_MSB': 0x00,
    'FILTER_2': 0x03,
...
DATA_TYPE_ADC = 0x80    # output raw ADC codes (after compensation & calibration)
...
#            # if the MI48 is streaming, it's obviously booted up before
        ...
        assert data_type in ['adc', 'temperature']
    ...
    def read(self):
        ...
        For `adc` or `temperature`in dK, the output is np.uint16.
        ...
        if self.capture_no_header:
            ...
            if self.mCFilter is not None:
                if self.parse_header:
                    ...
                    self.mCFilter.CheckSenxor(data, header[10])
    ...
    def set_data_type(self, data_type):
        """
        Set data type of the unit, from ['adc', 'temperature']
        """
        assert data_type in ['adc', 'temperature', ]
        self.data_type = data_type
    ...
    def get_camera_info(self, update=True):
        ...
        if self.module_type == 22:
            ...
            # this is despite the fact that the self.ncols = 50
